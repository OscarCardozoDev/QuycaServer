...
--
-- QUÉ HACE
-- Siembra los 45 estilos de la categoría `artes`. Una sola vez para toda la
...
--   sudo podman exec -i quyca_db psql -U prisma -d quyca < QuycaServer/scripts/dev.db.sql
--
-- ANTES hay que correr el seed estático, que siembra las cinco `GroupCategory`.
-- Sin la categoría `artes` este script no inserta nada y tampoco falla: el JOIN
-- no encuentra a qué colgar los estilos.
--
-- El nombre puede repetirse ENTRE categorías —"Contemporáneo" puede existir en
-- Danzas y en Música— pero no dentro de una. Por eso el conflicto se resuelve
-- contra `(name, categoryId)` y no contra `name` solo.
--
-- Las otras cuatro categorías todavía no tienen catálogo: sus estilos se cargan
-- desde el dashboard, con una sesión `super_admin`.
